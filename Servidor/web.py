...
def registro():
    ...
    #Generamos la salt y el hash irreversible
    salt = bcrypt.gensalt()
    password_hash = bcrypt.hashpw(password_plana, salt)
    ...
    try:
        ...
        conn.execute("PRAGMA journal_mode = WAL;")
        cursor = conn.cursor()
        #Guardamos el hash en la base de datos
        ...
        #Si funciona el registro, redireccionamos el usuario al panel
        return redirect(url_for('index'))
